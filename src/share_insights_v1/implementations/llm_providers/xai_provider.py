...
class XAIProvider(ILLMProvider):
    ...
    def __init__(self, model_name: str = "grok-beta"):
        ...
        debug_print(f"[XAI_DEBUG] API key found: {bool(self.api_key)}, model: {model_name}")
    
    def generate_response(self, prompt: str, **kwargs) -> str:
        """Generate response using XAI Grok"""
        if not self.api_key:
            debug_print("XAI API Key not found")
    ...
    def is_available(self) -> bool:
        ...
        debug_print(f"[XAI_DEBUG] is_available: {available}, api_key: {self.api_key is not None}")
        return available
    ...
    def set_current_model(self, model: str) -> bool:
        ...
        try:
            ...
            debug_print(f"[XAI_DEBUG] Switched to model {model}")
            return True
        except Exception as e:
